...
export default function MTalksForm() {
  ...
  const mTalksData = mTalksEvent && mTalksEvent[0];
  ...
  const form = useForm<mTalksSchema>({
    ...
    defaultValues: {
      participantName: mTalksData?.participantName ?? '',
      isITB: mTalksData?.isITB ?? false,
      nimITB: mTalksData?.nimITB ?? '',
      majorITB: mTalksData?.majorITB ?? '',
      institution: mTalksData?.institution ?? '',
      talksSessions: mTalksData?.talksSessions ?? [],
      followIgUrl: mTalksData?.followIgUrl ?? '',
      sourceInfo: mTalksData?.sourceInfo ?? undefined,
    },
  });
                                ...
                                <FormControl>
                                  <Checkbox
                                    ...
                                    checked={field.value?.includes(
                                      session.value as MTalksSessionType,
                                    )}
                                    onCheckedChange={(checked) => {
                                      return checked
                                        ? field.onChange([...field.value, session.value])
                                        : field.onChange(
                                            field.value?.filter((v) => v !== session.value),
                                          );
                                    }}
                                  />
                                </FormControl>
