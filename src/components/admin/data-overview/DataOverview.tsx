...
import { ActivityIcon, Clock4, FileClockIcon, HeartHandshakeIcon, MicVocalIcon, MotorbikeIcon, UserCheck2Icon, Users } from "lucide-react";
...
export default function DataOverview() {
    ...
    const mCareEvents = eventRegistrations?.filter((event) => event.eventType === "M_CARE")
    const etuEvents = eventRegistrations?.filter((event) => event.eventType === "ETU")
    const mTalksEvents = eventRegistrations?.filter((event) => event.eventType === "M_TALKS")
    const mRunEvents = eventRegistrations?.filter((event) => event.eventType === "M_RUN")
        ...
        <section>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                ...
                <div className="p-6 flex flex-col border rounded-lg">
                    ...
                    <p className="text-2xl">{eventRegistrations?.length}</p>
                ...
                <div className="p-6 flex flex-col border rounded-lg">
                    <div className="flex justify-between mb-4">
                        <h1 className="line-clamp-1">Total M-CARE Event</h1>
                        <HeartHandshakeIcon className="w-6 h-6" />
                    </div>
                    <p className="text-2xl">{mCareEvents?.length}</p>
                    <p className="text-muted-foreground text-sm">
                        Participants
                    </p>
                </div>
                <div className="p-6 flex flex-col border rounded-lg">
                    <div className="flex justify-between mb-4">
                        <h1 className="line-clamp-1">Total ETU Event</h1>
                        <MotorbikeIcon className="w-6 h-6" />
                    </div>
                    <p className="text-2xl">{etuEvents?.length}</p>
                    <p className="text-muted-foreground text-sm">
                        Participants
                    </p>
                </div>
                <div className="p-6 flex flex-col border rounded-lg">
                    <div className="flex justify-between mb-4">
                        <h1 className="line-clamp-1">Total M-TALKS & M-EXPO Event</h1>
                        <MicVocalIcon className="w-6 h-6" />
                    </div>
                    <p className="text-2xl">{mTalksEvents?.length}</p>
                    <p className="text-muted-foreground text-sm">
                        Participants
                    </p>
                </div>
                <div className="p-6 flex flex-col border rounded-lg">
                    <div className="flex justify-between mb-4">
                        <h1 className="line-clamp-1">Total M-RUN Event</h1>
                        <ActivityIcon className="w-6 h-6" />
                    </div>
                    <p className="text-2xl">{mRunEvents?.length}</p>
                    <p className="text-muted-foreground text-sm">
                        Participants
                    </p>
                </div>
