...
export const dashboardRateLimitMiddleware = t.middleware(
  async ({ ctx, next, path }) => {
    const identifier = ctx.session?.user.id || ctx.ip;

    const { success, reset } = await dashboardLimiter.limit(
      `${path}:${identifier}`,
    );
...
export const registerRateLimitMiddleware = t.middleware(
  async ({ ctx, next, path }) => {
    const identifier = ctx.session?.user.id || ctx.ip;

    const { success, reset } = await registerLimiter.limit(
      `${path}:${identifier}`,
    );
...
export const registerProctectedRateLimitedProcedure = t.procedure
  .use(({ ctx, next }) => {
    if (!ctx.session?.user) {
      throw new TRPCError({
        message: "Unauthorized to access this resource",
